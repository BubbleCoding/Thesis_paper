\documentclass[12pt]{report}

% -------------------------------
% Core packages (pdfLaTeX setup)
% -------------------------------
\usepackage[a4paper]{geometry}
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc} % UTF-8 input for pdfLaTeX

\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{fancyhdr}
\usepackage{float}
\usepackage{graphicx}
\usepackage[table,xcdraw]{xcolor}
\usepackage{hyperref}
\usepackage{appendix}
\usepackage[printonlyused]{acronym}
\usepackage{multirow}
\usepackage{url}
\usepackage{subfig}
\usepackage{siunitx}
\usepackage[acronym]{glossaries}
\usepackage{afterpage}
\newcommand{\questionref}[1]{\hyperref[#1]{\ref*{#1}}}
\hypersetup{
    hidelinks
}
% -------------------------------
% Listings (single, UTF-8 safe)
% -------------------------------
\usepackage{listings}
\usepackage{listingsutf8} % allow UTF-8 inside listings

% Global defaults for all code blocks
\lstset{
  inputencoding=utf8,
  upquote=true,
  showstringspaces=false,
  breaklines=true,
  breakatwhitespace=false,
  columns=fullflexible,
  basicstyle=\ttfamily\small,
  numbers=left,
  numberstyle=\tiny,
  numbersep=5pt,
  frame=single,
  % Map tricky Unicode so pdfLaTeX won't choke inside code
  literate=
    {▁}{{\_}}1
    {–}{{-}}1
    {—}{{-}}1
    {…}{{...}}3
    {🧠}{{[brain]}}1
    {✅}{{[check]}}1
    {⚠️}{{[warn]}}1
    {❌}{{[x]}}1
}

% Fallbacks for any stray Unicode outside listings or missed cases
\DeclareUnicodeCharacter{2581}{\_}       % ▁ (SentencePiece underscore)
\DeclareUnicodeCharacter{2705}{[check]}  % ✅
\DeclareUnicodeCharacter{26A0}{[warn]}   % ⚠
\DeclareUnicodeCharacter{274C}{[x]}      % ❌
\DeclareUnicodeCharacter{1F9E0}{[brain]} % 🧠
\DeclareUnicodeCharacter{FE0F}{}         % emoji variation selector (strip)

% -------------------------------
% Your custom listings languages/styles
% -------------------------------
\lstdefinelanguage{NetLogo}{
    basicstyle=\small,
    breaklines=true,
    breakatwhitespace=true,
    alsoletter={-,?,.},
    morekeywords=[1]{to, to-report, end, __includes, breed, globals, pseudopodia-own, foods-own, tubes-own, searchers-own, patches-own},
    keywordstyle=[1]\color[rgb]{0.0, 0.66, 0.42},
    morekeywords=[2]{ask, let, set, if, report, ifelse, clear-all, reset-ticks, die, create-link-with, face, fd, setxy, pen-down, hatch},
    keywordstyle=[2]\color{blue},
    morekeywords=[3]{patch, patches, xcor, ycor, of, random-float, with, any?, one-of, turtle-set, turtles-on, patch-ahead, pcolor, ranodm, not, can-move?, length, in-radius, item, neighbors, abs, pxcor, max-pxcor, pycor, max-pycor, size, myself, shabe, color, other, link-with, length, replace-item, map, list, and, or, not, n-of, min-pxcor, min-pycor, label-color, myself, sqrt, max, distance, insert-item, shape, hidden?, label},
    keywordstyle=[3]\color[rgb]{0.74, 0.2, 0.64},
    morekeywords=[4]{nobody},
    keywordstyle=[4]\color[rgb]{0.86, 0.08, 0.24},
    comment=[l]{\;},
    commentstyle=\color[rgb]{0.75,0.75,0.75},
    string=[d]{"},
    stringstyle=\color{orange},
    morekeywords=[5]{1, 2, 3, 4, 5, 6, 7, 8, 9, 0, true, false},
    keywordstyle=[5]{\color{orange}},
}

\lstdefinestyle{jsonstyle}{
  basicstyle=\ttfamily\small,
  breaklines=true,
  breakatwhitespace=false,
  columns=flexible,
  frame=single,
  language={}
}

\lstdefinestyle{pythonstyle}{
    language=Python,
    basicstyle=\ttfamily\small,
    breaklines=true,
    breakatwhitespace=false,
    numbers=left,
    numberstyle=\tiny,
    keywordstyle=\color{blue},
    commentstyle=\color{gray},
    stringstyle=\color{orange},
    showstringspaces=false,
    frame=single
}

% -------------------------------
% Misc settings
% -------------------------------
\newcommand\blankpage{%
\null \thispagestyle{empty}%
  \addtocounter{page}{-1}%
    \newpage}

\sloppy
\graphicspath{{images/}}

\setlength{\textwidth}{15 true cm}
\setlength{\textheight}{22 true cm}
\setlength{\headheight}{15pt}
\oddsidemargin  0.5 cm
\evensidemargin 0.5 cm
\topmargin      -1.0 cm

\urlstyle{tt}
\urldef{\test}{\url{http://de.wikipedia.org/wiki/Hilfe:TeX}}

\widowpenalty=10000
\clubpenalty=10000

\begin{document}

\include{titlepage}
\pagestyle{fancy}
\pagenumbering{roman}
\rfoot{\thepage}
\cfoot{}

% NOTE: If you don't have stmt.tex, comment the next line.
\include{stmt}

\include{erklaerung}
\include{abstract}
%\include{abstract_DE}
%\include{acknowledgement}
\include{AIUsage}
%\include{acronyms}
\newpage
\tableofcontents
\listoffigures
\listoftables
\clearpage

\pagestyle{plain}

\chapter{Introduction}
\label{sec:intro}
\pagenumbering{arabic}
\input{chapter1}

\chapter{Theoretical Research and Related Work}
\label{sec:relatedwork}
\input{chapter2}

\chapter{Research Methodology}
\label{sec:methods}
\input{chapter3}

\chapter{Artefact Development}
\label{sec:implementation}
\input{chapter4}

\chapter{Evaluation}
\label{sec:experiments}
\input{chapter5}

\chapter{Conclusion}
\label{sec:conclusion}
\input{chapter6}

\newpage

\bibliographystyle{IEEEtran}
\addcontentsline{toc}{chapter}{Bibliography}
\bibliography{bibfile_fixed(1)}

\newpage

\appendix
\addcontentsline{toc}{chapter}{APPENDICES}
\chapter{Artifact Code}
\label{sec:Artifact Code}
\input{appendix_a}
\chapter{Questionnaire}
\label{sec:Questionnaire}
\input{appendix_b}

\end{document}
